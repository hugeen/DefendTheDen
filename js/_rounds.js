(function() {
    ...
    DTD.loadLevel = function(level) {
        DTD.player = Crafty.e("Wolf");
        DTD.skills = [];
        DTD.skills["DTD.throwingAxeSkill"] = DTD.throwingAxeSkill();
        DTD.selectedSkill == "DTD.throwingAxeSkill";
        if(!level) {
            $("#progressBar .empty, #progressBar, #progressBarBelow").remove();
            $("body").append("<div id='endlessInGameScore' class='battleFieldUI'>0</div>")
            DTD.skills["DTD.blowSkill"] = DTD.blowSkill();
            DTD.skills["DTD.rockSkill"] = DTD.rockSkill();
            Crafty.e("Endless");
        } else {
            if(level > 1) {
                DTD.skills["DTD.blowSkill"] = DTD.blowSkill();
                    ...
                    if(this._endAt + 1000 + this._roundPause > now) {
                        ...
                        _.each(this.waves, function(item, key) {
                            if((item.at * 1000) + self._roundPause < now - start) {
                                if(!item.played) {
                                    item.played = true;
                                    var wave = DTD.waveParser(item.wave);
                                    for( i = 0; i < 6; i++) {
                                        if(wave[i] != "Nobody") {
